...
## 四、与 Decepticon 项目的关系

本系统参考了 [Decepticon](https://github.com/PurpleAILAB/Decepticon) (PurpleAILAB) 项目的多智能体红队架构。Decepticon 是一个面向 **网络基础设施渗透测试** 的 AI 红队系统, 使用 LangGraph Swarm 架构协调 4 个 Agent (Planner / Reconnaissance / Initial_Access / Summary) 在 Docker 容器中执行 nmap、hydra 等安全工具。

### 与 Decepticon 的对比

| 维度 | 本系统 | Decepticon |
|------|--------|------------|
| **定位** | 大模型安全态势感知 + 防御 | 通用红队自动化测试 |
| **闭环** | 攻防一体 (红队 → 蓝队 → 反馈迭代) | 纯攻击侧 |
| **特色** | 自动训练检测模型, 持续进化 | 社区知识共享 Replay |
| **目标** | 针对 LLM/AI 系统的安全 | 针对传统 IT 基础设施 |
| **攻击手段** | Prompt 注入、越狱、对抗样本、多模态攻击 | nmap, hydra, metasploit |
| **漏洞类型** | Prompt 泄露、安全护栏绕过、幻觉利用 | CVE、弱口令、配置错误 |
| **执行环境** | HTTP 请求 + 攻击提示词库 + 沙盒容器 | Docker 中的 Kali Linux |

### 可复用的组件与设计思路

| Decepticon 组件 | 本系统的复用方式 |
|----------------|----------------|
| Prompt 分层系统 (base + persona + coordination) | 直接复用分层 Prompt 架构 |
| Handoff 机制 (`create_handoff_tool` + `Command`) | 在各子系统内部的 Agent 间使用 |
| MCP 工具协议 (FastMCP Server) | 改造为 HTTP 攻击工具 / 爬虫工具 / 沙盒监控工具 |
| 共享记忆 (langmem) | 在子系统内部的 Agent 间共享情报 |
| 日志回放系统 (Session Logging) | 复用于攻击过程的记录与回放 |
| `create_react_agent` 模式 | 所有 Agent 统一使用 ReAct 范式 |
...
```
                ┌──────────────────┐
                │   Intel          │
                │   Supervisor     │  ← 分析覆盖率, 决定爬取策略
                └──┬───┬───┬──┬──┘
                   │   │   │  │
        ┌──────────┘   │   │  └──────────┐
        ▼              ▼   ▼             ▼
 ┌────────────┐ ┌──────────┐ ┌─────────┐ ┌──────────────┐
 │ Web Crawler │ │ Paper    │ │ Dark    │ │ Standardizer │
 │ Agent       │ │ Analyzer │ │ Web     │ │ Agent        │
 │             │ │ Agent    │ │ Agent   │ │              │
 │ - CVE/NVD  │ │ - arXiv  │ │ - 论坛   │ │ - 去重       │
 │ - MITRE    │ │ - 安全报告│ │ - TG群组 │ │ - STIX 2.1  │
 │ - GitHub   │ │ - PoC代码 │ │         │ │ - 写入知识库  │
 │ - HuggingFace│ │        │ │         │ │              │
 └────────────┘ └──────────┘ └─────────┘ └──────────────┘
```
...
```
                    ┌────────────────────┐
                    │   Red Team         │
                    │   Orchestrator     │  ← 从 attack_pool 取攻击, 分派给专家
                    └──┬───┬───┬───┬───┘
                       │   │   │   │
          ┌────────────┘   │   │   └────────────┐
          ▼                ▼   ▼                ▼
  ┌─────────────────┐ ┌────────────┐ ┌─────────────────┐ ┌─────────────────┐
  │ Prompt Injection │ │  Jailbreak │ │  Info Leakage    │ │  Multimodal     │
  │     Agent        │ │   Agent    │ │    Agent         │ │  Attack Agent   │
  │                  │ │            │ │                  │ │                 │
  │ - 直接/间接注入   │ │ - DAN/越狱  │ │ - 系统提示词提取  │ │ - 图像对抗样本  │
  │ - 上下文操纵      │ │ - 角色扮演  │ │ - 训练数据探测   │ │ - 音频攻击      │
  │ - 指令覆盖        │ │ - 编码绕过  │ │ - 配置信息泄露   │ │ - 跨模态注入    │
  └────────┬─────────┘ └─────┬──────┘ └────────┬────────┘ └────────┬────────┘
           │                 │                  │                   │
           └─────────────────┴────────┬─────────┴───────────────────┘
                                      ▼
